...
// Função para fazer login com Google OAuth2
exports.loginGoogle = async (req, res) => {
    const { tokenId } = req.body; // Recebe o token de ID do Google enviado no corpo da requisição

    try {
        // Verifica o token com a biblioteca do Google
        const ticket = await client.verifyIdToken({
            idToken: tokenId,
            audience: process.env.GOOGLE_CLIENT_ID, // Verifica se o token é para o seu app
        });

        const payload = ticket.getPayload(); // Obtém as informações do usuário
        ...
        if (!usuario) {
            ...
            usuario = await User.create({
                email: payload.email,
                senha: null, // Não precisa de senha aqui, pois o login é feito com Google
            });
        }
